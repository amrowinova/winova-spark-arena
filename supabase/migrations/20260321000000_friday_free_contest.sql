-- ============================================================
-- مسابقة الجمعة المجانية — Friday Free Contest
-- ============================================================
-- التغييرات:
-- 1. أعمدة جديدة في contests: is_free, admin_prize
-- 2. عمود جديد في contest_entries: device_fingerprint
-- 3. إضافة friday_prize إلى app_settings.contest_config
-- 4. تحديث join_contest RPC لدعم الدخول المجاني مع 3 طبقات حماية
-- ============================================================

-- 1. جدول contests — أعمدة جديدة
ALTER TABLE public.contests
  ADD COLUMN IF NOT EXISTS is_free        BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS admin_prize    NUMERIC DEFAULT NULL;

COMMENT ON COLUMN public.contests.is_free     IS 'TRUE للمسابقات المجانية (مسابقة الجمعة)';
COMMENT ON COLUMN public.contests.admin_prize IS 'الجائزة الثابتة للمسابقات المجانية — محددة من الأدمن';

-- 2. جدول contest_entries — بصمة الجهاز
ALTER TABLE public.contest_entries
  ADD COLUMN IF NOT EXISTS device_fingerprint TEXT DEFAULT NULL;

COMMENT ON COLUMN public.contest_entries.device_fingerprint IS 'بصمة الجهاز لمنع الانضمام المتكرر في المسابقات المجانية';

-- index لفحص التكرار بسرعة
CREATE INDEX IF NOT EXISTS idx_contest_entries_fingerprint
  ON public.contest_entries (contest_id, device_fingerprint)
  WHERE device_fingerprint IS NOT NULL;

-- 3. app_settings — أضف friday_prize إلى contest_config
UPDATE public.app_settings
SET value = value || jsonb_build_object('friday_prize', 100)
WHERE key = 'contest_config';

-- إذا لم يوجد contest_config بعد، أضفه
INSERT INTO public.app_settings (key, value, description)
SELECT
  'contest_config',
  '{"entryFee": 10, "prizePoolRate": 6, "voteEarningsPct": 20, "friday_prize": 100}'::jsonb,
  'إعدادات المسابقة اليومية'
WHERE NOT EXISTS (SELECT 1 FROM public.app_settings WHERE key = 'contest_config');

-- ============================================================
-- 4. تحديث join_contest RPC
-- ============================================================
-- نضيف parameter جديد: p_device_fingerprint TEXT DEFAULT NULL
-- الـ DEFAULT NULL يضمن أن الكود القديم لا ينكسر
-- ============================================================
...
BEGIN
  ...
  IF v_contest.is_free THEN
    ...
    -- الطبقة الثالثة: Device Fingerprint — جهاز واحد لكل مسابقة مجانية
    ...
    -- ✓ الانضمام المجاني — بدون خصم Nova
    ...
    -- تحديث عدد المشاركين فقط (prize_pool يبقى = admin_prize)
    UPDATE contests
    SET current_participants = current_participants + 1
    WHERE id = p_contest_id;

    -- لا wallet_ledger ولا transactions للمسابقات المجانية
