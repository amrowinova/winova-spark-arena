...
-- Fix: Set auth context before calling execute_transfer so auth.uid() works
...
BEGIN
  ...
  FOR v_i IN 1..p_transfer_count LOOP
    BEGIN
      ...
      -- Set auth context so auth.uid() = sender
  ...
  -- Reset to service role for remaining operations
  PERFORM set_config('role', 'postgres', true);

  -- PHASE 2: MESSAGE FLOOD (direct insert bypassing RLS for load test)
  BEGIN
