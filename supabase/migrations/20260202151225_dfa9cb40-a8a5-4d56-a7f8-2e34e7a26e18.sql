-- 1) Ensure required extensions exist for scheduling
create extension if not exists pg_net;
create extension if not exists pg_cron;

-- 2) Add contest_date to enforce one contest per day (KSA date)
...
create unique index if not exists contests_contest_date_unique
on public.contests (contest_date);

-- 3) Harden join_contest: must be real active contest + within join window (10:00-18:00 KSA) + for today's KSA date
...
begin
  ...
  if v_contest.contest_date <> v_ksa_today then
    return jsonb_build_object('success', false, 'error', 'No active contest');
  end if;
