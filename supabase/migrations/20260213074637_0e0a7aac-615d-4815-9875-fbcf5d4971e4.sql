...
BEGIN
  ...
  INTO v_wallets
  FROM (SELECT user_id, id, nova_balance FROM wallets WHERE nova_balance >= 2 ORDER BY nova_balance DESC LIMIT 30) w;
  ...
  -- PHASE 1: TRANSFER STORM (set JWT claims for auth.uid())
  FOR v_i IN 1..p_transfer_count LOOP
    BEGIN
      ...
      PERFORM set_config('request.jwt.claims', json_build_object('sub', v_sender_id, 'role', 'authenticated')::text, true);
      ...
      ELSE
        ...
        v_err_msg := v_transfer_result ->> 'error';
      END IF;
    EXCEPTION 
      WHEN deadlock_detected THEN v_deadlock_count := v_deadlock_count + 1; v_transfer_fail := v_transfer_fail + 1;
      WHEN lock_not_available THEN v_lock_wait_count := v_lock_wait_count + 1; v_transfer_fail := v_transfer_fail + 1;
      WHEN OTHERS THEN v_transfer_fail := v_transfer_fail + 1; GET STACKED DIAGNOSTICS v_err_msg = MESSAGE_TEXT;
    END;
  END LOOP;
  ...
  SELECT count(*) INTO v_desync_count FROM wallets w
  WHERE w.nova_balance <> (SELECT coalesce(sum(l.amount), 0) FROM wallet_ledger l WHERE l.wallet_id = w.id AND l.currency = 'nova');
  SELECT count(*) INTO v_stuck_escrow FROM p2p_orders
  WHERE status IN ('matched'::p2p_order_status, 'awaiting_payment'::p2p_order_status, 'payment_sent'::p2p_order_status)
  AND created_at < now() - interval '24 hours';

  RETURN jsonb_build_object(
    'simulation', jsonb_build_object('intensity', p_intensity, 'duration_ms', round(extract(milliseconds from clock_timestamp() - v_start_time)), 'wallets_used', v_wallet_count, 'transfers_requested', p_transfer_count),
    'transfer_storm', jsonb_build_object('successes', v_transfer_success, 'failures', v_transfer_fail, 'deadlocks', v_deadlock_count, 'lock_waits', v_lock_wait_count,
      'success_rate', round(v_transfer_success::numeric / GREATEST(p_transfer_count, 1) * 100, 1) || '%',
      'latency_min_ms', round(v_min_ms, 1), 'latency_avg_ms', round(v_total_ms / GREATEST(p_transfer_count, 1), 1), 'latency_max_ms', round(v_max_ms, 1), 'last_error', v_err_msg),
    'messaging_flood', jsonb_build_object('inserted', v_msg_success),
    'notification_spike', jsonb_build_object('inserted', v_notif_success),
    'integrity', jsonb_build_object('nova_supply_before', v_total_nova_before, 'nova_supply_after', v_total_nova_after,
      'supply_drift', abs(v_total_nova_after - v_total_nova_before), 'supply_conserved', abs(v_total_nova_after - v_total_nova_before) < 0.01,
      'wallet_ledger_desync', v_desync_count, 'stuck_escrows', v_stuck_escrow),
    'verdict', jsonb_build_object(
      'no_money_mismatch', abs(v_total_nova_after - v_total_nova_before) < 0.01 AND v_desync_count = 0,
      'no_deadlocks', v_deadlock_count = 0, 'no_stuck_escrow', v_stuck_escrow = 0, 'acceptable_latency', v_max_ms < 10000,
      'OVERALL', CASE WHEN abs(v_total_nova_after - v_total_nova_before) < 0.01 AND v_desync_count = 0 AND v_deadlock_count = 0 AND v_stuck_escrow = 0 AND v_max_ms < 10000 THEN 'PASS' ELSE 'ISSUES_DETECTED' END)
  );
END;
$$;
